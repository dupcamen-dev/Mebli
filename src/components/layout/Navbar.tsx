    ...
    >
      <nav className="flex justify-between items-center max-w-[1600px] mx-auto px-5 md:px-8 h-[76px]">
        <div className="flex items-center gap-4">
          ...
          >
            <img src="/logo.webp" alt="Mebli Chortkiv" className="h-full max-h-[56px] md:max-h-[60px] w-auto py-1" />
          </Link>
        </div>

        <div className="hidden md:flex gap-10 items-center">
          {!isInnerPage && c.links.map((link) => (
            <a
              key={link.href}
              href={link.href}
      ...
      <div className={`md:hidden fixed inset-0 top-0 bg-green-section transition-all duration-500 ease-in-out ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col items-center justify-between h-full px-8 pt-[100px] pb-10 overflow-y-auto">
          <div className="flex flex-col items-center gap-1">
            {!isInnerPage && c.links.map((link, i) => (
              <a
                key={link.href}
                href={link.href}
                onClick={(e) => handleClick(e, link.href)}
                className="text-white hover:text-tertiary transition-colors duration-300 cursor-pointer font-[family-name:var(--font-headline)] text-[24px] font-medium py-3.5"
            ...
            {isAdmin && (
              <Link href="/admin" onClick={close} className="text-tertiary hover:text-white transition-colors duration-300 font-[family-name:var(--font-headline)] text-[24px] font-medium py-3.5"
                style={{ transitionDelay: mobileOpen ? `${c.links.length * 80}ms` : "0ms", opacity: mobileOpen ? 1 : 0, transform: mobileOpen ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.4s ease, transform 0.4s ease, color 0.3s ease" }}>
                {c.adminPanel}
              </Link>
            )}
          </div>
          ...
          <div className="flex flex-col items-center gap-5 w-full max-w-[280px]">
            {!isInnerPage && (
              <a href="#contact" onClick={(e) => handleClick(e, "#contact")} className="w-full bg-white text-green-section px-10 py-4 text-[14px] font-bold uppercase tracking-[0.15em] font-[family-name:var(--font-body)] hover:bg-white/90 transition-all duration-300 cursor-pointer rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.25)] text-center"
                style={{ transitionDelay: mobileOpen ? `${(c.links.length + 2) * 80}ms` : "0ms", opacity: mobileOpen ? 1 : 0, transform: mobileOpen ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.4s ease, transform 0.4s ease, background-color 0.3s ease" }}>
                {c.cta}
              </a>
            )}
            {session ? (
              <div className="flex flex-col items-center gap-4 w-full" style={{ transitionDelay: mobileOpen ? `${(c.links.length + 3) * 80}ms` : "0ms", opacity: mobileOpen ? 1 : 0, transform: mobileOpen ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.4s ease, transform 0.4s ease" }}>
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => { signIn("google"); close(); }} className="w-full flex items-center justify-center gap-2.5 bg-white text-green-section px-6 py-3.5 text-[14px] font-bold uppercase tracking-[0.1em] font-[family-name:var(--font-body)] hover:bg-white/90 transition-all duration-300 cursor-pointer rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.25)]"
                style={{ transitionDelay: mobileOpen ? `${(c.links.length + 3) * 80}ms` : "0ms", opacity: mobileOpen ? 1 : 0, transform: mobileOpen ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.4s ease, transform 0.4s ease, background-color 0.3s ease" }}>
                <svg className="w-[18px] h-[18px]" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                {c.signInGoogle}
              </button>
            )}
